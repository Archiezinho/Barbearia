...
$sql = ("SELECT * from agenda where hora = '$hora' and barbeiro = '$barbeiro'");
    ...
    if($rows == 0){
        ...
        echo "<script>
        alert('Seu corte está agendado para o dia $data as $hora, entraremos em contato através do
        seu número $telefone para confirmação...');

        window.location.href='http://localhost/barbearia_novo/index.php'</script>";

    }else{
        echo "<script>
        alert('Já tem um cliente agendado $hora com o barbeiro $barbeiro, tente agendar com 
        outro barbeiro ou em outro horario!');
